...
module.exports = function(config) {
  ...
  function setupRoutes(app, folder, controllers) {
    for (var name in controllers) {
      if(controllers.hasOwnProperty(name)) {
        if(_.isString(controllers[name])) {
          var controller = require(folder + '/' + name);
          var endpoint = controllers[name];
          app.use(endpoint, controller.router());
        } else if (_.isObject(controllers[name])) {
          setupRoutes(app, folder, controllers[name]);
        }
      }
    }
  }

  setupRoutes(app, config.folders.controllers, config.controllers);
